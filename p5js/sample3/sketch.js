...
function draw() {
  ...
  let eyeMoveX = map(mouseX, 0, width, -3, 3);
  let eyeMoveY = map(mouseY, 0, height, -2, 2);
  ...
  fill(30); 
  noStroke();
  circle(160 + eyeMoveX, 310 + eyeMoveY, 10); // 마우스에 따라 눈동자 이동
  circle(240 + eyeMoveX, 310 + eyeMoveY, 10);
  ...
  if (mouth === 0) {
    ...
    arc(200, 380, 40, 20, 0, PI);
  } 
  else if (mouth === 2) {
    arc(200, 395, 40, 20, PI, TWO_PI);
  }
  noStroke(); //주근깨
  fill(160, 120, 90, 150); 
...
function keyPressed() {
  if (key === ' ') {
    mouth = (mouth + 1) % 3;
    console.log("현재 상태:", mouth);
  }
  if (key === 's' || key === 'S') { 
    saveGif('myCharacter_task3', 10); 
  }
}
